...
Betrachten nun mehrere ZV'en gleichzeitig.\\
Fragen:
\begin{itemize}
\item Haben ZV'en „Einfluss aufeinander“?\\
...
\cparagraph{Definition} Sind $X$ und $Y$ ZV'en, so heißt 
$$F: \RR^2 \to [0,1], \; F(a,b) = \PP(X \leq a, Y \leq b)$$
gemeinsame Verteilungsfunktion von $X$ und $Y$.
\begin{itemize}
\item Sind beide ZV'en diskret, wobei $X$ die Werte $x_1, x_2, \dots$ und $Y$ die Werte $y_1, y_2, \dots$ annehmen kann, dann heißt die Funktion $f$ gegeben durch 
$$f(x_i, y_j):= \PP(X=x_i, Y=y_j)$$
\emph{gemeinsame Verteilungsfunktion}.
\item Sind beide ZV'en stetig und existiert eine Funktion $f: \RR^2 \to [0,\infty)$ mit 
...
\begin{itemize}
\item Die Definition 1.2.30 lässt sich auf beliebig viele ZV'en erweitern.
\item ZV'en lassen sich zu einem (zufälligen) Vektor zusammenfassen:
\end{itemize}

\cparagraph{Definition} Sind $X_1, \dots, X_n$ ZV'en so heißt
$$\mtr{X_1\\X_2\\\vdots\\X_n}$$
\emph{n-dimensionaler Zufallsvektor}.

\cparagraph{Beispiel} (Zufallsvektor mit $n=2$)\\
Seien $X,Y$ diskrete ZV'en gegeben durch\\
...
Dann \tab $\PP(X=0)=\frac{1}{2}$ \tab $\PP(X=1)=\frac{1}{4}$\\
$ $\tab $\PP(X=2) = \frac{1}{4}$ \tab $\PP(X=0) = \frac{1}{4}$\\
$ $\tab $\PP(Y=1) = \frac{1}{4}$ \tab $\PP(Y=2) = \frac{1}{2}$\\
$ $\tab $\PP(X\cdot Y=0) = \frac{1}{2}$ \tab $\PP(X\cdot Y = 2) = \frac{1}{2}$

...
\item $\int_{-\infty}^{\infty}f(x)\intd{x}=1$ (Dichtefunktion ist normiert)
\end{itemize}
...
Es muss gelten $1\overset{!}{=}\int_{-\infty}^{\infty}f(x)\intd{x}=\int_0^{10}h\intd{x}=\left[ h\cdot x\right]_0^{10}=10\cdot h \Rightarrow h = \frac{1}{10}$
\item Wie sieht $F_X$ aus?\\
ABB S8\\
$F_X(x)=\int_{-\infty}^x f(t) \intd{t}=\begin{cases}
0 & x\leq 0\\
\tfrac{1}{10} x & 0 < x < 10\\
1 & x \geq 10
\end{cases}$\\
Fall $x\leq 0$: $F_X(x)=\int_{-\infty}^x 0 \intd{t} = 0$\\
Fall $0<x<10$: $F_X(x)= \int_{-\infty}^0 0 \intd{t} + \int_0^x \frac{1}{10} \intd{t} = \frac{1}{10}x$\\
Fall $x \geq 10$: $F_X(x) = \underbrace{\int_{-\infty}^0 0 \intd{t}}_{0}+ \underbrace{\int_0^{10} \frac{1}{10} \intd{t}}_1 + \underbrace{\int_{10}^x 0 \intd{t}}_0 = 1$
\end{anumerate}
